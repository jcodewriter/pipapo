...
if($('#nectar_fullscreen_rows').length > 0 && $disableFPonMobile != 'on' || $().fullpage && $disableFPonMobile != 'on') {
	...
	function fullscreenRowLogic() {
		$('.full-page-inner-wrap .full-page-inner > .span_12 > .wpb_column').each(function(){
			if($(this).find('> .column-inner > .wpb_wrapper').find('> .wpb_row').length > 0) {

				//add class for css
				$(this).find('> .column-inner > .wpb_wrapper').addClass('only_rows');

				//set number of rows for css
				$rowNum = $(this).find('> .column-inner > .wpb_wrapper').find('> .wpb_row').length;
				$(this).find('> .column-inner > .wpb_wrapper').attr('data-inner-row-num',$rowNum);
			} 
	...
	function fullHeightRowOverflow() {
		...
		if($(window).width() >= 1000) {
	    	...
	    	$('#nectar_fullscreen_rows > .wpb_row .full-page-inner-wrap[data-content-pos="full_height"]').each(function(){
	    		...
	    		$(this).find('> .full-page-inner > .span_12 > .wpb_column > .column-inner > .wpb_wrapper').each(function(){
	...
	$('body').on('click','.tabbed > ul li:not(.cta-button) a',function(){
		...
		if($tabbedClickCount != 0) {
			...
			if($(this).parents('.wpb_row').length > 0) {
				if($(this).parents('.tabbed').find('> div:nth-of-type('+$id+')').find('.pie_chart').length > 0  ||
		...
		$(this).parents('.tabbed').find('.wpb_row').each(function(){
			if(typeof $(this).find('[class*="col-"]').first().offset() != 'undefined') {
				
				var $firstChildOffset = $(this).find('[class*="col-"]').first().offset().left;
...
function morphingOutlines() {
	...
	if($('.morphing-outline').length > 0) {
		...
		$('.morphing-outline').each(function(i){
			...
			$morphingOutlineCSS += 'body .wpb_column:hover > .wpb_wrapper > .morphing-outline.instance-'+i+' .inner > *, body .wpb_column:hover > .column-inner > .wpb_wrapper > .morphing-outline.instance-'+i+' .inner > * { color: '+$(this).attr("data-hover-color")+'; } ';
			$morphingOutlineCSS += 'body .wpb_column:hover > .wpb_wrapper > .morphing-outline.instance-'+i+' .inner:after, body .wpb_column:hover > .column-inner > .wpb_wrapper > .morphing-outline.instance-'+i+' .inner:after  { border-color: '+$(this).attr("data-hover-color")+'; } ';
			//padding calcs
			$morphingOutlineCSS += 'body .wpb_column'+$hover2+' > .wpb_wrapper > .morphing-outline.instance-'+i+' .inner:after, body .wpb_column'+$hover2+' > .column-inner > .wpb_wrapper > .morphing-outline.instance-'+i+' .inner:after { padding: '+(($width+100 + $border*2 - $height)/2 - $border) +'px 50px}';
			$morphingOutlineCSS += '.morphing-outline.instance-'+i+' { padding: '+(30+($width+80 + $border*2 - $height)/2 - $border) +'px 50px}'; //extra space on the outer for mobile/close elements
			$morphingOutlineCSS += 'body .wpb_column'+$hover2+' > .wpb_wrapper > .morphing-outline.instance-'+i+' .inner:after, body .wpb_column'+$hover2+' > .column-inner > .wpb_wrapper > .morphing-outline.instance-'+i+' .inner:after { top: -'+ parseInt((($width+100 + $border*2 - $height)/2 - $border) + $border)+ 'px }';

			$morphingOutlineCSS += 'body .wpb_column > .wpb_wrapper > .morphing-outline.instance-'+i+' .inner:after, body .wpb_column > .column-inner > .wpb_wrapper > .morphing-outline.instance-'+i+' .inner:after { left: -' + parseInt(50+$border) + 'px }';
			////hover
			$morphingOutlineCSS += 'body .wpb_column'+$hover+' > .wpb_wrapper > .morphing-outline.instance-'+i+' .inner:after, body .wpb_column'+$hover+' > .column-inner > .wpb_wrapper > .morphing-outline.instance-'+i+' .inner:after { padding: 50px 50px}';
			$morphingOutlineCSS += 'body .wpb_column'+$hover+' > .wpb_wrapper > .morphing-outline.instance-'+i+' .inner:after, body .wpb_column'+$hover+' > .column-inner > .wpb_wrapper > .morphing-outline.instance-'+i+' .inner:after { top: -'+parseInt(50+$border) +'px }';
...
/***************** Full Width Section ******************/
	...
	function fullWidthContentColumns(){
		...
		//standard carousel
			...
			if($(this).find('> .span_12 > .col').length > 1){
				...
				$(this).find('> .span_12 > .col').each(function(){
					...
					$column_inner_selector = ($(this).find('> .column-inner > .wpb_wrapper').length > 0) ? '.column-inner' : '.column-inner-wrap > .column-inner';
	    	 	...
	    	 	$(this).find('> .span_12 > .col').each(function(){
		    	 	...
		    	 	if($(this).find('> '+$column_inner_selector+' > .wpb_wrapper > *').length > 0){
		    	 		...
		    	 		if(!$(this).parent().parent().hasClass('row-o-equal-height')) $(this).css('height',tallestColumn);
		    	 	} 
		    	 	//empty columns
		    	 	else {
	         	...
	         	//vertically center
	         	if($(this).hasClass('vertically-align-columns') && window.innerWidth > 1000 && !$(this).hasClass('row-o-equal-height')){
		...
		$('.main-content > .row > .wpb_row:not(.full-width-content).row-o-equal-height').each(function(){
		...
		$('.row.row-o-equal-height>.span_12>.wpb_column[class*="padding-"][data-padding-pos="all"]').each(function(){
...
function cascadingImageBGSizing() {
	$('.nectar_cascading_images').each(function(){
		...
		if($(this).parents('.row-o-equal-height').length > 0 && $(this).parents('.wpb_column').length > 0) 
...
/***************** 4 Col Grid in iPad ******************/
	...
	function oneFourthClasses() {
		$('.col.span_3, .span3, .col-sm-3').each(function(){
			var $currentDiv = $(this);
			var $nextDiv = $(this).next('div');
			if( $nextDiv.hasClass('span_3') && !$currentDiv.hasClass('one-fourths') || $nextDiv.hasClass('span3') && !$currentDiv.hasClass('one-fourths') || $nextDiv.hasClass('col-sm-3') && !$currentDiv.hasClass('one-fourths') ) {
...
function imageWithHotspotClickEvents() {
	...
	$('body').on('click','.nectar_hotspot.click',function(){
		...
		if($(window).width() <= 690) $(this).parents('.wpb_row, [class*="col-"]').css('z-index','200');
...
/***************** Nectar Video BG ******************/
    ...
    function videoBGInit(){
	    ...
	    jQuery(".row").each(function() {
	        ...
	        $row.find('.nectar-youtube-bg').length > 0 ? (youtubeUrl = $row.find('.nectar-youtube-bg span').text(), youtubeId = nectarExtractYoutubeId(youtubeUrl), youtubeId && ($row.find(".video-bg").remove(), nectarInsertYoutubeVideoAsBackground($row.find('.nectar-youtube-bg'), youtubeId))) : $row.find(".nectar-youtube-bg").remove()
...
/*-------------------------------------------------------------------------*/	
	...
	function resizeInit() {
		 ...
		 //vc mobile columns
		 if($('[class*="col-xs-"], [class*="col-md-"], [class*="col-lg-"]').length > 0) vcMobileColumns();
	...
	function recentPostHeight() {
		$('.blog-recent[data-style="title_only"]').each(function(){
			...
			if($(this).parent().parent().parent().hasClass('col-sm-3') || 
				$(this).parent().parent().parent().hasClass('col-sm-4') || 
				$(this).parent().parent().parent().hasClass('col-sm-6') || 
				$(this).parent().parent().parent().hasClass('col-sm-8') || 
				$(this).parent().parent().parent().hasClass('col-sm-9')) {
		...
		function recentPostSliderHeight(){
			...
			$('.nectar-recent-posts-slider').each(function(){
						...
						if( window.innerWidth > 1000 && $('#boxed').length == 0) {
							...
							if(!$parentCol.hasClass('span12') && !$parentCol.hasClass('main-content') && !$parentCol.hasClass('span_12') && !$parentCol.hasClass('col-sm-12') ) {
		...
		if($('.infinite_scroll').length > 0) {
			...
			$('#post-area.infinite_scroll .posts-container').infinitescroll({
		   	...
		   	if($('.masonry.meta_overlaid').length == 0) { 
				...
				//pie chart		
			    if($().vcChat) $(newElements).find('.pie_chart').vcChat();
...
/*-------------------------------------------------------------------------*/	
	 ...
	 function crossBrowserFixes() {
		...
		if( $('.nectar-slider-wrap.first-section').length == 0 && 
			$('.full-width-section.first-section > .span_12 > .span12 > .wpb_wrapper > .nectar-slider-wrap').length == 0  && 
